...
if ($type == "pm") {
	...
	if ($config['mail_bcc']) {
		...
		foreach ($h_mail as $key => $email) {
			...
			$message_send = str_replace("{%unsubscribe%}", "--", $message);
		...
		elseif (strpos($config['http_home_url'], "/") === 0) $slink = "http://".$_SERVER['HTTP_HOST'].$config['http_home_url'];
		  ...
		  while( $row = $db->get_row() ) {
		    ...
		    $message_send = str_replace("{%unsubscribe%}", $slink . "index.php?do=newsletterunsubscribe&user_id=" . $row['user_id'] . "&hash=" . $hash, $message);
