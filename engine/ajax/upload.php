...
function insert_all() {
	...
	if (wysiwyg != 'no') {
		if ( wysiwyg == '1' ) {
			...
			if (align == 'center') { content = links.join('</p><p>&nbsp;</p><p style="text-align:center;">'); } else { content = links.join(' '); }
...
function buildthumb( image, mass, tag ) {
	...
	if( wysiwyg == '1' ) {
		...
		content = '<a href="'+image+'" rel="highslide" class="highslide" target="_blank">';
		...
		if (wysiwyg != 'no') {
			...
			if ( wysiwyg == '1' ) {
				...
				if (align == 'center' || align == 'none') {
					...
					content = '<img src="'+ image +'" style="float:' + align+ ';" alt="'+ imagealt +'" title="'+ imagealt +'" />';		
				}
