...
if( $allow_userinfo and $doaction == "adduserinfo" ) {
	...
	if( !$is_logged or !($member_id['user_id'] == $row['user_id'] or $member_id['user_group'] == 1) ) {
		...
		if( dle_strlen( $filecontents, $config['charset'] ) > 65000 ) $stop .= "<li>" .$lang['content_error'] . "</li>";
		if( dle_strlen( $info, $config['charset'] ) > 65000 ) $stop .= "<li>" .$lang['content_error'] . "</li>";
		if( dle_strlen( $signature, $config['charset'] ) > 65000 ) $stop .= "<li>" .$lang['content_error'] . "</li>";
		if( dle_strlen( $allowed_ip, $config['charset'] ) > 255 ) $stop .= "<li>" .$lang['content_error'] . "</li>";
	...
	if( $stop ) {
		...
		if( strlen( $password1 ) > 0 ) {
			...
			$sql_user = "UPDATE " . USERPREFIX . "_users SET fullname='$fullname', land='$land',{$mailchange} info='$info', signature='$signature', password='$password1', allow_mail='$allow_mail', xfields='$filecontents', allowed_ip='$allowed_ip', timezone='$timezone', news_subscribe='$news_subscribe', comments_reply_subscribe='$comments_reply_subscribe' WHERE user_id = '{$id}'";
		
		} else {
			
			$sql_user = "UPDATE " . USERPREFIX . "_users SET fullname='$fullname', land='$land',{$mailchange} info='$info', signature='$signature', allow_mail='$allow_mail', xfields='$filecontents', allowed_ip='$allowed_ip', timezone='$timezone', news_subscribe='$news_subscribe', comments_reply_subscribe='$comments_reply_subscribe' WHERE user_id = '{$id}'";
