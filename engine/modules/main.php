	...
	if( strpos( $tpl->copy_template, "xfvalue" ) !== false OR strpos( $tpl->copy_template, "[xfgiven_" ) !== false ) {
		...
		foreach ( $xfields as $value ) {
			...
			if($value[3] == "imagegalery" AND $xfieldsdata[$value[0]] ) {
				...
				$onload_scripts[] = "hs.addSlideshow({slideshowGroup: 'xf_".NEWS_ID."_{$value[0]}', interval: 4000, repeat: false, useControls: true, fixedControls: 'fit', overlayOptions: { opacity: .75, position: 'bottom center', hideOnMouseOut: true } });";
